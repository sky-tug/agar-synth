...
sids=sorted(plak,key=lambda s:plak[s]['n'])
for i,sid in enumerate(sids):
    ax=fig.add_subplot(gs[i//5,i%5]); p=plak[sid]
    ...
    for r in [x for x in rows if x['sid']==sid]:
        rr=max((r['w']+r['h'])/4, 16)
        ax.add_patch(Circle((r['x'],r['y']),rr,fc=C[r['sinif']],
...
# panel 1: esit alanli halkalarda yogunluk
ax=fig.add_subplot(gs[2,0:2]); kur(ax,"Radyal yoğunluk — eşit alanlı 5 halka","plak yarıçapının kesri  (r/R)")
rn=np.array([r['r_norm'] for r in rows]); kn=[math.sqrt(i/5) for i in range(6)]
pay=[100*((rn>=kn[i])&(rn<kn[i+1])).sum()/len(rn) for i in range(5)]
ax.bar(range(5),pay,color="#2a78d6",width=0.72)
ax.axhline(20,color=INK2,lw=1.6,ls=(0,(4,3)))
ax.text(4.42,26.5,"düzgün dağılım = %20",fontsize=8,color=INK2,ha="right")
ax.set_xticks(range(5)); ax.set_xticklabels([f"{kn[i]:.2f}–{kn[i+1]:.2f}" for i in range(5)],fontsize=7.5)
ax.set_ylabel("kolonilerin %'si",fontsize=8.5,color=INK2)
for i,v in enumerate(pay): ax.text(i,v+0.6,f"%{v:.1f}",ha="center",fontsize=8,color=INK)
ax.set_ylim(0,29)

# panel 2: en yakin komsu / cap
ax=fig.add_subplot(gs[2,2:4]); kur(ax,"Koloniler birbirine değiyor mu?","en yakın komşu mesafesi ÷ koloni çapı")
nn=[]
for sid in plak:
...
ax.text(1.08,ax.get_ylim()[1]*0.88,f"solunda = değiyor\n%{100*(nn<1).mean():.0f} koloni",
        fontsize=8.5,color="#eb6834")
ax.set_ylabel("koloni sayısı",fontsize=8.5,color=INK2)

# panel 3: Clark-Evans
ax=fig.add_subplot(gs[2,4]); kur(ax,"Yerleşim rastgele mi?","Clark–Evans oranı")
ce=[]
for sid in sids:
    rs=[r for r in rows if r['sid']==sid]
    if len(rs)<5: continue
    p=plak[sid]; P=np.array([[r['x'],r['y']] for r in rs])
    D=np.hypot(P[:,None,0]-P[None,:,0],P[:,None,1]-P[None,:,1]); np.fill_diagonal(D,1e9)
...
ax.text(1.0,-0.95,"rastgele",fontsize=8,color=INK2,ha="center")
ax.text(0.775,-0.95,"kümelenmiş",fontsize=8,color=INK2)
ax.text(1.245,-0.95,"düzenli",fontsize=8,color=INK2,ha="right")
...
fig.text(0.045,0.955,"AGAR yerleşim keşfi — 10 demo plak, 387 koloni",fontsize=15,color=INK,weight="bold")
fig.text(0.045,0.925,"Kesikli daire = plak yarıçapının %90'ı. Daire boyutu gerçek koloni boyutuyla orantılı (görünürlük için alt sınır uygulandı).",
         fontsize=9.5,color=INK2)
fig.savefig("/home/claude/faz3/yerlesim_kesfi.png",dpi=135,facecolor=SURF)
print("ok")
